...
def calculate_fed_avg_local_dataset_inversion_performance(architecture_layout, dataset, max_client_dataset_size,
                                                          ...
                                                          tolerance_map, n_samples, config, max_n_cpus, first_cpu, device,state_name=0):
    ...
    algo_name="inversion"  # tableak, inversion
    experiment_name="normal" # normal, dp, fair, fairdp
    
    with open(f'clients_data/reconstr_and_GT/{algo_name}/{experiment_name}/dataset_{state_name}.pkl', 'wb') as f:
        pickle.dump(dataset, f)

    with open(f'clients_data/reconstr_and_GT/{algo_name}/{experiment_name}/tolerance_map_{state_name}.pkl', 'wb') as f:
    ...
    for i, lepochs in enumerate(local_epochs):
        for j, lbatch_size in enumerate(local_batch_sizes):
            for k, epoch_prior_param in enumerate(epoch_prior_params):
                ...
                print("Input to Model: ",dataset.num_features, architecture_layout)
                ...
                # with open(f'clients_data/reconstr_and_GT/{algo_name}/{experiment_name}/reconstructions_ground_truths_{state_name}.pkl', 'wb') as f:
                ...
                # print("reconstructions_and_ground_truths is dumped--OFF")
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--name_state', type=int, nargs='+', default=[0], help='List of State Codes')
    parser.add_argument('--name_state', type=int, default=0, help='State Code')
    parser.add_argument('--random_seed', type=int, default=42, help='Set the random state for reproducibility')
    parser.add_argument('--max_n_cpus', type=int, default=2, help='The number of available cpus')
